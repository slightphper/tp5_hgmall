...
function addToCartShowDiv(goodsId, script_name,goods_recommend,parentId)
{
  ...
  if($(".goods_info_attr").length > 0){
	  if(attr_list == is_selected){
		  Ajax.call('flow.php?step=add_to_cart_showDiv1', 'goods=' + $.toJSON(goods), addToCartShowDivResponse, 'POST', 'JSON');
	  }else{
...
function addToCartResponse(result)
{
  ...
  {
    // var cart_url = ajax_cart_url;
    if (result.one_step_buy == '1')
    {
      // location.href = ajax_cart_url;
      // 重新开启一个页面，但是链接多次只是跳这一个页面
      window.open(ajax_cart_url,"win1");
    }
    else
    {
