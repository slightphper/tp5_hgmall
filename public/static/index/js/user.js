...
function userLogin()
{
  ...
  Ajax.call( check_login, 'username=' + username.val()+'&password='+password.val()+remember_string, return_login , 'POST', 'JSON');

	// Ajax.call( check_login, 'username=' + username.val()+'&password='+password.val()
  // +remember_string+'&captcha='+captcha.val()
  // +'&dsc_token='+dsc_token.val()+'&back_act='+back_act, return_login , 'POST', 'JSON');
}
...
function return_login(result)
{
	...
	else
	{
		...
		if(result.is_validated == 1){
			...
			location.href = shop_index;//登录成功，跳转首页
		}
	}
}
